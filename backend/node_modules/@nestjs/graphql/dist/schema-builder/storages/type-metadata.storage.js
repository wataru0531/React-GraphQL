...
class TypeMetadataStorageHost {
    constructor() {
        this.logger = new common_1.Logger(TypeMetadataStorageHost.name);
    ...
    resolveRegisterIn(registerIn) {
        ...
        try {
            ...
            this.logger.warn(`Failed to resolve registerIn function. The type will be included in all schemas (backward compatible behavior). Error: ${error}`);
            return undefined;
        }
    }
    shouldIncludeInModules(registerIn, moduleSet) {
    ...
    filterByModules(metadata, modules) {
        const moduleSet = new Set(modules);
        return metadata.filter((item) => this.shouldIncludeInModules(item.registerIn, moduleSet));
    }
    getObjectTypesMetadataByModules(modules) {
        return this.filterByModules(this.metadataByTargetCollection.all.objectType, modules);
    }
    getInputTypesMetadataByModules(modules) {
        return this.filterByModules(this.metadataByTargetCollection.all.inputType, modules);
    }
    getInterfacesMetadataByModules(modules) {
        return this.filterByModules([...this.metadataByTargetCollection.all.interface.values()], modules);
    }
    getArgumentsMetadataByModules(modules) {
        return this.filterByModules(this.metadataByTargetCollection.all.argumentType, modules);
    }
    getEnumsMetadataByModules(modules) {
        return this.filterByModules(this.enums, modules);
    }
    getUnionsMetadataByModules(modules) {
    ...
    applyPluginMetadata(prototype, loadedClasses = new Set()) {
        do {
            ...
            const metadata = Object.getOwnPropertyDescriptor(prototype.constructor, plugin_constants_1.METADATA_FACTORY_NAME)?.value?.();
            if (!metadata) {
                continue;
            }
    ...
    getInheritedClassDirectives(target) {
        const ownDirectives = this.metadataByTargetCollection
            .get(target)
            .classDirectives.getAll();
        const inherited = [];
        const seenSdls = new Set(ownDirectives.map((d) => d.sdl));
    ...
    compileExtendedResolversMetadata() {
        let queries = this.queries, mutations = this.mutations, subscriptions = this.subscriptions;
        this.metadataByTargetCollection.all.resolver.forEach((item) => {
            ...
            while (parentClass.prototype) {
                ...
                if (parentMetadata) {
                    queries = this.mergeParentResolverHandlers(queries, parentClass, item);
                    mutations = this.mergeParentResolverHandlers(mutations, parentClass, item);
                    subscriptions = this.mergeParentResolverHandlers(subscriptions, parentClass, item);
