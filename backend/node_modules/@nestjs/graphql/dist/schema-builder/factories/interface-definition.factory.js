...
const interface_not_registered_error_1 = require("../errors/interface-not-registered.error");
...
let InterfaceDefinitionFactory = class InterfaceDefinitionFactory {
    ...
    generateInterfaces(metadata, getParentType) {
        const prototype = Object.getPrototypeOf(metadata.target);
        return () => {
            const interfaces = (0, get_interfaces_array_util_1.getInterfacesArray)(metadata.interfaces).map((item) => {
                const definition = this.typeDefinitionsStorage.getInterfaceByTarget(item);
                if (!definition) {
                    throw new interface_not_registered_error_1.InterfaceNotRegisteredError(metadata.name, item);
                }
