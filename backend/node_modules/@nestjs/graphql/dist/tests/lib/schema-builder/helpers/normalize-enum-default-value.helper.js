...
exports.normalizeEnumDefaultValue = normalizeEnumDefaultValue;
 ...
 */
function normalizeEnumDefaultValue(defaultValue, typeRef, enumsMetadata) {
    if ((0, shared_utils_1.isUndefined)(defaultValue) || defaultValue === null) {
        return defaultValue;
    }
    ...
    const enumRef = enumMetadata.ref;
    if (Array.isArray(defaultValue)) {
        return defaultValue.map((item) => translateEnumValue(item, enumRef));
    }
    return translateEnumValue(defaultValue, enumRef);
}
function translateEnumValue(value, enumRef) {
...
function getEnumMemberValues(enumRef) {
    const keys = Object.keys(enumRef).filter((key) => isNaN(parseInt(key, 10)));
    return keys.map((key) => enumRef[key]);
}
//# sourceMappingURL=normalize-enum-default-value.helper.js.map