"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const graphql_1 = require("graphql");
const scalar_types_utils_1 = require("../../lib/utils/scalar-types.utils");
describe('createScalarType', () => {
    class BaseScalar {
        constructor() {
            this.description = 'Date custom scalar type';
        }
        parseValue(value) {
            return typeof value === 'number' ? new Date(value) : null;
        }
        serialize(value) {
            return value instanceof Date ? value.getTime() : null;
        }
        parseLiteral(ast) {
            if (ast.kind === graphql_1.Kind.INT) {
                return new Date(parseInt(ast.value, 10));
            }
            return null;
        }
    }
    it('should propagate description, parseValue, serialize and parseLiteral', () => {
        const scalar = (0, scalar_types_utils_1.createScalarType)('Date', new BaseScalar());
        expect(scalar.name).toBe('Date');
        expect(scalar.description).toBe('Date custom scalar type');
        expect(scalar.parseValue(0)).toEqual(new Date(0));
        expect(scalar.serialize(new Date(0))).toBe(0);
        expect(scalar.parseLiteral({ kind: graphql_1.Kind.INT, value: '0' }, {})).toEqual(new Date(0));
    });
    it('should propagate specifiedByURL when provided by the instance', () => {
        class DateTimeScalar extends BaseScalar {
            constructor() {
                super(...arguments);
                this.specifiedByURL = 'https://scalars.graphql.org/andimarek/date-time.html';
            }
        }
        const scalar = (0, scalar_types_utils_1.createScalarType)('DateTime', new DateTimeScalar());
        expect(scalar.specifiedByURL).toBe('https://scalars.graphql.org/andimarek/date-time.html');
    });
    it('should propagate extensions when provided by the instance', () => {
        class TaggedScalar extends BaseScalar {
            constructor() {
                super(...arguments);
                this.extensions = { tag: 'public' };
            }
        }
        const scalar = (0, scalar_types_utils_1.createScalarType)('Tagged', new TaggedScalar());
        expect(scalar.extensions).toEqual({ tag: 'public' });
    });
    it('should leave specifiedByURL and extensions at GraphQL defaults when not set', () => {
        const scalar = (0, scalar_types_utils_1.createScalarType)('Plain', new BaseScalar());
        expect(scalar.specifiedByURL).toBeUndefined();
        expect(scalar.extensions).toEqual({});
    });
});
//# sourceMappingURL=scalar-types.util.spec.js.map